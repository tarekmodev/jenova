...
describe.skipIf(!available)("fan-out migration runner", () => {
  ...
  it("isolates a failing tenant, reports per-tenant status, and resumes on re-run", async () => {
    ...
    await writeFile(path.join(migrationsDir, "0002_fanout_probe.sql"), "create table fanout_probe (id int primary key);\n");
    ...
    for (const tenant of dry.tenants) {
      expect(tenant.pending).toEqual(["0002_fanout_probe.sql"]);
    ...
    expect(resumed.get(slugs[1] ?? "")?.applied).toEqual(["0002_fanout_probe.sql"]);
  ...
  it("refuses edited (checksum-changed) applied migrations", async () => {
    ...
    await writeFile(path.join(migrationsDir, "0002_fanout_probe.sql"), "-- edited after apply\nselect 1;\n");
    const sql = connectTenant(dbName);
    try {
